// ContrastPanel.jsx — Untrained vs Full ARBITER side-by-side with shared timeline scrubber

window.ContrastPanel = function ContrastPanel({ trajectoryData }) {
  const { useState } = React;

  // Synthetic trajectories — will be swapped with real JSON data via trajectoryData prop
  const SEED = 42;
  ...
  const renderClaim = (claim, active) => {
    ...
    return (
      <div key={claim.step} className={`claim-card ${active ? (claim.correct ? 'correct' : 'incorrect') : ''}`}
        style={{ opacity: active ? 1 : 0.25, transition: 'all 0.3s', marginBottom: '6px',
          borderLeftColor: !active ? 'var(--border-light)' : undefined,
        }}>
  ...
  const renderSide = (label, subtitle, trajectory, score, verdictCorrect, dimmed) => (
    <div className="card" style={{
      ...
      <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <span style={{
            ...
            background: verdictCorrect ? 'rgba(0,196,224,0.1)' : 'var(--bg-shell)',
            color: verdictCorrect ? 'var(--cyan)' : 'var(--text-muted)',
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{
            ...
            color: verdictCorrect ? 'var(--green)' : 'var(--text-muted)',
          }}>{score.toFixed(1)}</div>
      ...
      {currentStep >= maxStep && (
        <div style={{
          margin: '0 20px 20px', padding: '14px 18px', borderRadius: '12px',
          background: verdictCorrect ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)',
          border: `1px solid ${verdictCorrect ? 'var(--green)' : 'var(--red)'}`,
        }}>
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '24px', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--text-muted)', marginTop: '2px' }}>
            Same episode (seed {SEED}) — different agent capabilities
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: '16px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <input id="contrast-slider" type="range" min={0} max={maxStep} value={currentStep}
            onChange={e => setCurrentStep(Number(e.target.value))}
            style={{ flex: 1, accentColor: 'var(--cyan)', height: '4px', cursor: 'pointer' }}
          />
          <button onClick={() => setCurrentStep(maxStep)} style={{
        ...
        <div style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'var(--text-muted)', textAlign: 'center', marginTop: '6px' }}>
          Drag to scrub — compare decision quality at each step
        </div>
      </div>
    </div>
  );
